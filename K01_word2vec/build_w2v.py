...
from K01_word2vec.data_utils import dump_pkl
...
def build(train_x_seg_path, test_y_seg_path, test_seg_path, out_path=None, sentence_path='',
    ...
    w2v = （one line）
    """
    w2v = Word2Vec(sentences=sentences, size=256, sg=1)
...
if __name__ == '__main__':
    build('F:\开课吧\自然语言处理\数据\\train_set.seg_x.txt'.format(BASE_DIR),
          'F:\开课吧\自然语言处理\数据\\train_set.seg_y.txt'.format(BASE_DIR),
          'F:\开课吧\自然语言处理\数据\\test_set.seg_x.txt'.format(BASE_DIR),
          out_path='F:\开课吧\自然语言处理\数据\\word2vec.txt'.format(BASE_DIR),
          sentence_path='F:\开课吧\自然语言处理\数据\\sentences.txt'.format(BASE_DIR))
